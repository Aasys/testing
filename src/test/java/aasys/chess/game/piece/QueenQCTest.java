...
public class QueenQCTest {
    ...
    public void allRangeTest(@InRange(minChar = 'a', maxChar = 'h') char xBoard,
        ...
        BasePiece piece = PieceFactory.createPiece(chessBoard, player, ChessPiece.QUEEN, new Position(positionString));

        Set<Position> positions = StraightMover.getMoves(piece);
        positions.addAll(DiagonalMover.getMoves(piece));

        assert positions.containsAll(piece.getValidMoves());
    }
